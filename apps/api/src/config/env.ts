...
const environmentSchema = z.object({
  ...
  GOOGLE_ANALYTICS_PROPERTY_ID: z.string().trim().regex(/^\d+$/).optional(),
  GOOGLE_ANALYTICS_MEASUREMENT_ID: z.string().trim().regex(/^G-[A-Z0-9]+$/).optional(),
  GOOGLE_ANALYTICS_CLIENT_EMAIL: z.string().trim().email().optional(),
  GOOGLE_ANALYTICS_PRIVATE_KEY: z.string().trim().min(1).optional(),
  GOOGLE_ANALYTICS_TIMEOUT_MS: z.coerce.number().int().positive().max(30_000).default(10_000),
