...
router.post('/login', validateRequest(loginSchema), asyncHandler(async (request, response) => {
  ...
  await Promise.all([
    ...
    recordSuccessfulLogin(request, user, { channel: 'web', platform: 'web' })
  ]);
