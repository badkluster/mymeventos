...
export const dashboardMetricDefinitions: MetricDefinition[] = [
  ...
  { id: 'leads.pending', label: 'Leads pendientes', description: 'Leads del período que todavía requieren gestión comercial.', source: 'Lead', formula: 'count(Lead.createdAt within period AND status in new, contacted, follow_up, quote_sent, negotiation)', attributionDate: 'createdAt', format: 'integer', drillDownHref: '/admin/leads?status=pending' },
  ...
  { id: 'events.upcoming', label: 'Eventos próximos', description: 'Eventos activos de los próximos 30 días desde hoy.', source: 'Event', formula: 'count(Event.eventDate between today and today + 30 days AND active status)', attributionDate: 'eventDate', format: 'integer', drillDownHref: '/admin/events?scope=upcoming' },
  { id: 'contracts.total', label: 'Total contratado', description: 'Suma de contratos no cancelados creados en el período.', source: 'Contract', formula: 'sum(Contract.totalAmount where createdAt within period)', attributionDate: 'createdAt', format: 'currency', financial: true, drillDownHref: '/admin/reports/contracts' },
  { id: 'payments.collected', label: 'Total cobrado', description: 'Pagos confirmados cobrados durante el período, netos de devoluciones.', source: 'Payment', formula: 'sum(paid payments) - sum(paid refunds)', attributionDate: 'paidAt', format: 'currency', financial: true, drillDownHref: '/admin/reports/payments?status=paid' },
  { id: 'payments.overdue', label: 'Saldo vencido', description: 'Cuotas pendientes cuya fecha de vencimiento ya pasó.', source: 'Payment', formula: 'sum(Payment.amount where status = pending AND dueDate < now)', attributionDate: 'dueDate', format: 'currency', financial: true, drillDownHref: '/admin/reports/payments?aging=overdue' },
  { id: 'expenses.paid', label: 'Gastos registrados', description: 'Gastos pagados dentro del período.', source: 'Expense', formula: 'sum(Expense.amount where status = paid AND paidAt within period)', attributionDate: 'paidAt', format: 'currency', financial: true, drillDownHref: '/admin/reports/expenses' },
