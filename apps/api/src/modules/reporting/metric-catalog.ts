...
export const dashboardMetricDefinitions: MetricDefinition[] = [
  ...
  { id: 'payments.averageTicket', label: 'Ticket promedio', description: 'Valor promedio de los contratos aprobados durante el período.', source: 'Contract', formula: 'sum(Contract.totalAmount where status = approved AND approvedAt within period) / count(same)', attributionDate: 'approvedAt', format: 'currency', financial: true, drillDownHref: '/admin/reports/contracts?attribution=approved&status=approved' },
  { id: 'finance.profitMargin', label: 'Margen de rentabilidad', description: 'Porcentaje de lo cobrado en el período que queda como margen después de los gastos pagados.', source: 'Payment + Expense', formula: '(cobrado - gastos pagados) / cobrado * 100', attributionDate: 'paidAt', format: 'percentage', financial: true, drillDownHref: '/admin/expenses/profitability' },
  { id: 'production.readiness', label: 'Producción lista', description: 'Porcentaje de eventos activos del período con producción chequeada o cerrada.', source: 'ProductionPlan', formula: '(eventos activos - producciones pendientes) / eventos activos * 100', attributionDate: 'eventDate', format: 'percentage', drillDownHref: '/admin/production' },
];
