...
export async function sendEmail(input: EmailInput): Promise<void> { const client = getTransporter(); if (!client) { console.info('Email skipped: SMTP is disabled or not configured.'); return; } await client.sendMail({ from: env.SMTP_FROM ?? env.SMTP_USER, ...input }); }
