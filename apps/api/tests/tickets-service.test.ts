...
vi.mock('../src/modules/tickets/ticket.models', () => ({
  TicketSale: { findOneAndUpdate: mocks.saleFindOneAndUpdate, updateOne: mocks.saleUpdateOne },
...
const sale = { _id: '507f1f77bcf86cd799439011', eventId: '507f1f77bcf86cd799439012', salonId: '507f1f77bcf86cd799439013', status: 'active', capacity: 1, reservedCount: 0, soldCount: 0, maxTicketsPerOrder: 2, allowFreeTickets: true };
...
describe('ticket capacity and QR concurrency', () => {
  ...
  it('admits only one concurrent reservation when the last seat is requested', async () => {
    ...
    const input = (idempotencyKey: string) => reservePublicOrder({ sale, buyer: { name: 'Ana', email: 'ana@example.com' }, selections: [{ ticketTypeId: '507f1f77bcf86cd799439014', quantity: 1 }], idempotencyKey });
  ...
  it('consumes a QR only once through the conditional valid-to-used transition', async () => {
    ...
    const [first, second] = await Promise.all([claimTicketCheckIn('507f1f77bcf86cd799439012', 'secure-ticket-token-123456789', '507f1f77bcf86cd799439016'), claimTicketCheckIn('507f1f77bcf86cd799439012', 'secure-ticket-token-123456789', '507f1f77bcf86cd799439016')]);
  ...
  it('immediately converts a free reservation into sold capacity', async () => {
    ...
    await reservePublicOrder({ sale: { ...sale, capacity: 5 }, buyer: { name: 'Ana', email: 'ana@example.com' }, selections: [{ ticketTypeId: '507f1f77bcf86cd799439014', quantity: 1 }], idempotencyKey: 'free-order-idempotency-001' });
