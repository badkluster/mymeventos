...
import { contractAddendumStatusLabels, contractStatusLabels, displayLabel, eventTypeLabels } from '@/lib/display-labels';
...
export default function ContractDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const load = async (contractId: string) => {
    ...
    try {
      ...
      setConsiderations(contractResponse.contract.considerations ?? '');
  ...
  const save = async () => {
    ...
    try {
      ...
      await api.patch(`/contracts/${contract._id}`, { observations, considerations, legalTermsSnapshot: { ...contract.legalTermsSnapshot, clauses } });
      await load(id);
  ...
  const submitCancel = async () => {
    if (!contract || !cancelReason.trim()) return showToast({ message: 'Indicá el motivo de la cancelación.', variant: 'error' });
    setSaving(true);
    try {
      await api.post(`/contracts/${contract._id}/cancel`, { reason: cancelReason });
      await load(id);
      showToast({ message: 'Contrato cancelado.', variant: 'success' });
      setShowCancelModal(false);
      setCancelReason('');
    } catch (error) {
      showToast({ message: error instanceof Error ? error.message : 'No se pudo cancelar el contrato.', variant: 'error' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const closePreview = () => { setPreviewOpen(false); setPreviewUrl((current) => { if (current) URL.revokeObjectURL(current); return ''; }); };
  const previewPdf = async () => {
    if (!contract) return;
    setPreviewOpen(true);
    setPreviewLoading(true);
    try {
      const { blob } = await api.download(`/contracts/${contract._id}/preview-pdf`);
      setPreviewUrl((current) => { if (current) URL.revokeObjectURL(current); return URL.createObjectURL(blob); });
    } catch (error) {
      showToast({ message: error instanceof Error ? error.message : 'No se pudo generar la vista previa.', variant: 'error' });
  ...
  return <section className="space-y-6 pb-8">
    ...
    {activeTab === 'evento' && <Card title="Evento"><Item label="Tipo" value={displayLabel(eventTypeLabels, text(contract.eventSnapshot?.eventType))} /><Item label="Agasajado/a" value={text(contract.eventSnapshot?.honoreeName)} /><Item label="Fecha" value={formatDate(contract.eventSnapshot?.eventDate)} /><Item label="Horario" value={[contract.eventSnapshot?.startTime, contract.eventSnapshot?.endTime].filter(Boolean).join(' - ') || 'Sin horario'} /><Item label="Invitados" value={String(contract.eventSnapshot?.guestCount ?? 'Sin definir')} /><Item label="Restricciones alimentarias" value={`Vegetarianos: ${contract.eventSnapshot?.vegetarianCount ?? 0} · Veganos: ${contract.eventSnapshot?.veganCount ?? 0} · Celíacos: ${contract.eventSnapshot?.celiacCount ?? 0} · Sin lactosa: ${contract.eventSnapshot?.lactoseIntolerantCount ?? 0}`} /><Item label="Mantelería" value={text(contract.eventSnapshot?.tableLinenColor)} /><Item label="Salón" value={text(contract.eventSnapshot?.salonName)} /></Card>}
    ...
    {activeTab === 'actividad' && <Card title="Consideraciones y actividad"><label className="block text-sm font-medium text-zinc-700">Consideraciones<Textarea value={considerations} onChange={(event) => setConsiderations(event.target.value)} className="mt-2 min-h-32" /><span className="mt-1 block text-xs font-normal text-zinc-500">Se incluyen completas en el PDF del contrato, sin recortes.</span></label><label className="mt-5 block text-sm font-medium text-zinc-700">Observaciones internas<Textarea value={observations} onChange={(event) => setObservations(event.target.value)} className="mt-2 min-h-32" /></label><Button disabled={saving} onClick={() => void save()}><Save className="mr-2 h-4 w-4" />Guardar consideraciones y observaciones</Button></Card>}
    <Modal open={previewOpen} title={`Vista previa · ${contract.contractNumber}`} description={contract.status === 'approved' ? 'Documento aprobado, generado a partir de los datos guardados.' : 'Documento generado al vuelo con los datos actuales, sin guardarlo: no reemplaza el PDF definitivo que se genera al aprobar.'} onClose={closePreview}>
      <div className="h-[72vh] min-h-[440px] bg-zinc-100">{previewLoading ? <div className="grid h-full place-items-center p-8 text-sm text-zinc-500">Generando vista previa...</div> : previewUrl ? <iframe title={`Vista previa ${contract.contractNumber}`} src={previewUrl} className="h-full w-full border-0" /> : null}</div>
    </Modal>
    <Modal open={showCancelModal} title="Cancelar contrato" description="El contrato cancelado no admite nuevos pagos ni adendas. Los pagos y adendas ya existentes se conservan." onClose={() => setShowCancelModal(false)}>
      <div className="space-y-4 p-5 sm:p-8">
        <label className="block text-sm font-medium text-zinc-700">Motivo (obligatorio)
          <Textarea className="mt-2" value={cancelReason} onChange={(event) => setCancelReason(event.target.value)} placeholder="Explicá por qué se cancela el contrato." />
        </label>
        <div className="flex justify-end gap-2">
          <Button variant="secondary" disabled={saving} onClick={() => setShowCancelModal(false)}>Volver</Button>
          <Button variant="danger" disabled={saving} onClick={() => void submitCancel()}>Confirmar cancelación</Button>
        </div>
      </div>
    </Modal>
  </section>;
