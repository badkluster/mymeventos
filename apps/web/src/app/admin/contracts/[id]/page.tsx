...
import { AlertTriangle, CheckCircle2, ChevronLeft, CreditCard, Plus, Printer, RotateCcw, Save, XCircle } from 'lucide-react';
...
export default function ContractDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const tabs = ['resumen', 'cliente', 'evento', 'servicios', 'menu', 'valores', 'adendas', 'condiciones', 'pagos', 'actividad'];
  ...
  return <section className="space-y-6 pb-8">
    ...
    <header className="rounded-3xl border border-zinc-200 bg-white px-6 py-6 shadow-sm md:px-8"><div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between"><div><div className="flex flex-wrap items-center gap-3"><h1 className="text-3xl font-semibold tracking-tight text-zinc-950">{contract.contractNumber}</h1><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${statusTone[contract.status] ?? 'bg-zinc-100 text-zinc-700'}`}>{displayLabel(contractStatusLabels, contract.status)}</span></div><p className="mt-2 text-sm text-zinc-500">{text(contract.customerSnapshot?.fullName)} · {text(contract.eventSnapshot?.eventType, 'Evento')}</p></div><div className="flex flex-wrap gap-2"><Button disabled={saving || contract.status === 'approved' || contract.status === 'cancelled'} onClick={() => void action('approve', 'Contrato aprobado.')}><CheckCircle2 className="mr-2 h-4 w-4" />Aprobar</Button><Button variant="secondary" disabled={saving || contract.status === 'cancelled'} onClick={() => void action('request-changes', 'Contrato marcado con cambios requeridos.')}><AlertTriangle className="mr-2 h-4 w-4" />Requiere cambios</Button><Button variant="secondary" disabled={saving || contract.status === 'cancelled'} onClick={() => void action('cancel', 'Contrato cancelado.')}><XCircle className="mr-2 h-4 w-4" />Cancelar</Button><Link href={`/admin/contracts/${contract._id}/print`}><Button variant="secondary"><Printer className="mr-2 h-4 w-4" />Imprimir</Button></Link></div></div></header>
    ...
    {activeTab === 'cliente' && <Card title="Cliente"><Item label="Nombre" value={text(contract.customerSnapshot?.fullName)} /><Item label="Documento" value={text(contract.customerSnapshot?.documentNumber ?? contract.customerSnapshot?.dni)} /><Item label="Teléfono" value={text(contract.customerSnapshot?.phone)} /><Item label="Email" value={text(contract.customerSnapshot?.email)} /><Item label="Domicilio" value={text(contract.customerSnapshot?.address)} /></Card>}
    {activeTab === 'evento' && <Card title="Evento"><Item label="Tipo" value={text(contract.eventSnapshot?.eventType)} /><Item label="Fecha" value={formatDate(contract.eventSnapshot?.eventDate)} /><Item label="Horario" value={[contract.eventSnapshot?.startTime, contract.eventSnapshot?.endTime].filter(Boolean).join(' - ') || 'Sin horario'} /><Item label="Invitados" value={String(contract.eventSnapshot?.guestCount ?? 'Sin definir')} /><Item label="Salón" value={text(contract.eventSnapshot?.salonName)} /></Card>}
    ...
    {activeTab === 'pagos' && <div className="space-y-5"><div className="grid gap-5 lg:grid-cols-4"><article className="rounded-2xl border border-zinc-200 bg-zinc-950 p-6 text-white shadow-sm"><p className="text-sm font-medium text-zinc-300">Saldo contractual</p><p className="mt-3 text-3xl font-semibold">{money(contract.balanceAmount)}</p><p className="mt-4 text-sm text-zinc-300">Total: {money(contract.totalAmount)}</p></article><Card title="Cobrado"><Item label="Impacta saldo" value={money(contract.paidAmount)} /><Item label="Reembolsado" value={money(paymentSummary.refundedAmount)} /></Card><Card title="Pendiente"><Item label="Cuotas pendientes" value={money(paymentSummary.pendingAmount)} /><Item label="Vencido" value={money(paymentSummary.overdueAmount)} /></Card><Card title="Garantía"><Item label="Depósito recibido" value={money(paymentSummary.securityDepositAmount)} /><Item label="No descuenta saldo" value="Sí" /></Card></div><Card title="Registrar pago"><div className="grid gap-3 lg:grid-cols-6"><Select value={newPayment.type} onChange={(event) => setNewPayment((current) => ({ ...current, type: event.target.value }))}>{Object.entries(paymentTypeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select><Select value={newPayment.status} onChange={(event) => setNewPayment((current) => ({ ...current, status: event.target.value }))}>{Object.entries(paymentStatusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select><Select value={newPayment.method} onChange={(event) => setNewPayment((current) => ({ ...current, method: event.target.value }))}>{Object.entries(paymentMethodLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select><Input type="number" min={0} value={newPayment.amount} onChange={(event) => setNewPayment((current) => ({ ...current, amount: Number(event.target.value) }))} placeholder="Importe" /><Input type="date" value={newPayment.dueDate} onChange={(event) => setNewPayment((current) => ({ ...current, dueDate: event.target.value }))} /><Button disabled={saving} onClick={() => void createNewPayment()}><CreditCard className="mr-2 h-4 w-4" />Registrar</Button></div><div className="grid gap-3 lg:grid-cols-[1fr_1fr_auto_auto]"><Input value={newPayment.reference} onChange={(event) => setNewPayment((current) => ({ ...current, reference: event.target.value }))} placeholder="Referencia / comprobante" /><Input value={newPayment.notes} onChange={(event) => setNewPayment((current) => ({ ...current, notes: event.target.value }))} placeholder="Notas" /><Button type="button" variant="secondary" onClick={() => setNewPayment((current) => ({ ...current, type: 'deposit', status: 'paid' }))}>Seña</Button><Button type="button" variant="secondary" onClick={() => setNewPayment((current) => ({ ...current, type: 'security_deposit', status: 'paid' }))}>Garantía</Button></div></Card><Card title="Historial de pagos">{payments.length ? <div className="overflow-x-auto"><table className="min-w-[980px] w-full text-sm"><thead className="text-left text-xs uppercase text-zinc-400"><tr><th className="py-2">Número</th><th>Tipo</th><th>Estado</th><th>Medio</th><th>Importe</th><th>Fecha</th><th>Referencia</th><th className="text-right">Acciones</th></tr></thead><tbody className="divide-y divide-zinc-100">{payments.map((payment) => <tr key={payment._id}><td className="py-3"><Link className="font-medium text-zinc-950 underline" href={`/admin/payments/${payment._id}`}>{payment.paymentNumber}</Link></td><td>{displayLabel(paymentTypeLabels, payment.type)}</td><td>{displayLabel(paymentStatusLabels, payment.status)}</td><td>{payment.method ? displayLabel(paymentMethodLabels, payment.method) : 'No informado'}</td><td>{money(payment.amount)}</td><td>{formatDate(payment.paidAt ?? payment.dueDate)}</td><td>{payment.reference || payment.receiptNumber || 'Sin referencia'}</td><td><div className="flex justify-end gap-2"><Button variant="secondary" disabled={saving || payment.status === 'paid'} onClick={() => void paymentAction(payment, 'mark-paid', 'Pago marcado como cobrado.')}>Cobrar</Button><Button variant="secondary" disabled={saving || payment.status !== 'paid' || payment.type === 'refund'} onClick={() => void paymentAction(payment, 'refund', 'Reembolso registrado.')}><RotateCcw className="mr-2 h-4 w-4" />Reembolsar</Button><Button variant="secondary" disabled={saving || payment.status === 'cancelled'} onClick={() => void paymentAction(payment, 'cancel', 'Pago cancelado.')}>Cancelar</Button></div></td></tr>)}</tbody></table></div> : <Empty text="No hay pagos registrados para este contrato." />}</Card></div>}
