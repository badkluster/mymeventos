...
export default function ContractPrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto max-w-4xl bg-white p-8 text-zinc-950 print:p-0">
    ...
    <Section title="Datos de la prestadora"><p className="whitespace-pre-wrap">{text(contract.legalTermsSnapshot?.providerText, 'M&M Eventos · Servicios de salón, catering y organización integral de eventos.')}</p></Section>
    <Section title="Datos del usuario"><Grid items={[['Nombre', text(contract.customerSnapshot?.fullName)], ['Documento', text(contract.customerSnapshot?.documentNumber ?? contract.customerSnapshot?.dni)], ['Ocupación', text(contract.customerSnapshot?.occupation)], ['Teléfono', text(contract.customerSnapshot?.phone)], ['Email', text(contract.customerSnapshot?.email)], ['Domicilio', text(contract.customerSnapshot?.address)]]} /></Section>
    <Section title="Datos del evento"><Grid items={[['Tipo', displayLabel(eventTypeLabels, text(contract.eventSnapshot?.eventType))], ['Agasajado/a', text(contract.eventSnapshot?.honoreeName)], ['Fecha', formatDate(contract.eventSnapshot?.eventDate)], ['Horario', [contract.eventSnapshot?.startTime, contract.eventSnapshot?.endTime].filter(Boolean).join(' - ') || 'Sin horario'], ['Invitados', String(contract.eventSnapshot?.guestCount ?? 'Sin definir')], ['Vegetarianos', String(contract.eventSnapshot?.vegetarianCount ?? 0)], ['Veganos', String(contract.eventSnapshot?.veganCount ?? 0)], ['Celíacos', String(contract.eventSnapshot?.celiacCount ?? 0)], ['Intolerancia a lactosa', String(contract.eventSnapshot?.lactoseIntolerantCount ?? 0)], ['Mantelería', text(contract.eventSnapshot?.tableLinenColor)], ['Salón', text(contract.eventSnapshot?.salonName)], ['Dirección', text(contract.eventSnapshot?.salonAddress)]]} /></Section>
    ...
    <Section title="Valores y pagos"><Grid items={[['Paquete', text(contract.commercialSnapshot?.packageName, 'Personalizado')], ['Modalidad', contract.commercialSnapshot?.pricingMode === 'fixed' ? 'Precio total del evento' : 'Precio por persona'], [contract.commercialSnapshot?.pricingMode === 'fixed' ? 'Precio total acordado' : 'Valor final por persona', money(contract.commercialSnapshot?.pricingMode === 'fixed' ? contract.commercialSnapshot?.finalFixedPrice ?? contract.totalAmount : contract.commercialSnapshot?.finalPricePerPerson)], ['Base', money(contract.baseAmount)], ['Adendas aprobadas', money(contract.approvedAddendumsAmount)], ['Descuentos', money(contract.discountsAmount)], ['Total contractual', money(contract.totalAmount)], ['Seña acordada', money(contract.commercialSnapshot?.depositAmount)], ['Saldo', money(contract.balanceAmount)], ['Condiciones', text(contract.paymentAgreementSnapshot?.paymentTerms)], ['Depósito por roturas', money(contract.securityDeposit?.amount)]]} /></Section>
    ...
    <Section title="Consideraciones"><p className="whitespace-pre-wrap break-words">{contract.considerations || 'Sin consideraciones.'}</p></Section>
