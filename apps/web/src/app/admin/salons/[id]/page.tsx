...
type PackageStatusFilter = 'active' | 'inactive' | 'all';
type PackageScopeFilter = 'all' | 'global' | 'salon';
type RuleForm = {
  name: string;
...
export default function SalonDetailPage() {
  ...
  const [packageStatusFilter, setPackageStatusFilter] = useState<PackageStatusFilter>('active');
  const [packageScopeFilter, setPackageScopeFilter] = useState<PackageScopeFilter>('all');
  ...
  const packageRuleCounts = {
    all: packageRules.length,
    active: packageRules.filter((rule) => rule.active !== false).length,
    inactive: packageRules.filter((rule) => rule.active === false).length,
    global: packageRules.filter((rule) => rule.isGlobal).length,
    salon: packageRules.filter((rule) => !rule.isGlobal).length
  };
  const filteredPackageRules = packageRules.filter((rule) => {
    const matchesStatus = packageStatusFilter === 'all' || (packageStatusFilter === 'active' ? rule.active !== false : rule.active === false);
    const matchesScope = packageScopeFilter === 'all' || (packageScopeFilter === 'global' ? rule.isGlobal : !rule.isGlobal);
  ...
  return <section className="space-y-6">
    ...
    {tab === 'packages' && <div className="grid gap-4"><div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm"><div><h2 className="font-semibold text-zinc-950">Paquetes y reglas del salón</h2><p className="mt-1 text-sm text-zinc-500">Los paquetes nuevos se asignan sólo a {salon.name}. Creá uno global únicamente si debe estar disponible para todos los salones.</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => openTemplate('salon')}><Plus className="mr-2 h-4 w-4" />Nuevo paquete para este salón</Button><Button variant="secondary" onClick={() => openTemplate('global')}><Plus className="mr-2 h-4 w-4" />Nuevo paquete global</Button></div></div><section className="flex flex-wrap items-end justify-between gap-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-4" aria-label="Filtros de paquetes"><div><p role="status" aria-live="polite" className="text-sm font-medium text-zinc-950">Mostrando {filteredPackageRules.length} de {packageRuleCounts.all} paquetes</p><p className="mt-1 text-sm text-zinc-500">Por defecto se muestran los paquetes activos, globales y propios de este salón.</p></div><div className="flex flex-wrap gap-4"><div><p className="mb-1.5 text-sm font-medium text-zinc-700">Estado</p><div className="flex flex-wrap gap-1 rounded-xl border border-zinc-200 bg-white p-1" role="group" aria-label="Filtrar paquetes por estado"><button type="button" aria-pressed={packageStatusFilter === 'active'} onClick={() => setPackageStatusFilter('active')} className={packageFilterButtonClass(packageStatusFilter === 'active')}>Activos ({packageRuleCounts.active})</button><button type="button" aria-pressed={packageStatusFilter === 'inactive'} onClick={() => setPackageStatusFilter('inactive')} className={packageFilterButtonClass(packageStatusFilter === 'inactive')}>Inactivos ({packageRuleCounts.inactive})</button><button type="button" aria-pressed={packageStatusFilter === 'all'} onClick={() => setPackageStatusFilter('all')} className={packageFilterButtonClass(packageStatusFilter === 'all')}>Todos ({packageRuleCounts.all})</button></div></div><div><p className="mb-1.5 text-sm font-medium text-zinc-700">Origen</p><div className="flex flex-wrap gap-1 rounded-xl border border-zinc-200 bg-white p-1" role="group" aria-label="Filtrar paquetes por origen"><button type="button" aria-pressed={packageScopeFilter === 'all'} onClick={() => setPackageScopeFilter('all')} className={packageFilterButtonClass(packageScopeFilter === 'all')}>Todos ({packageRuleCounts.all})</button><button type="button" aria-pressed={packageScopeFilter === 'global'} onClick={() => setPackageScopeFilter('global')} className={packageFilterButtonClass(packageScopeFilter === 'global')}>Globales ({packageRuleCounts.global})</button><button type="button" aria-pressed={packageScopeFilter === 'salon'} onClick={() => setPackageScopeFilter('salon')} className={packageFilterButtonClass(packageScopeFilter === 'salon')}>Propios ({packageRuleCounts.salon})</button></div></div></div></section>{filteredPackageRules.map((rule) => {
    ...
    })}{!filteredPackageRules.length && <div className="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 px-6 py-10 text-center"><p className="text-sm font-medium text-zinc-900">No hay paquetes que coincidan con estos filtros.</p><p className="mt-1 text-sm text-zinc-500">Probá cambiar el estado u origen para ver otros paquetes.</p>{packageRuleCounts.all > 0 && <button type="button" onClick={() => { setPackageStatusFilter('all'); setPackageScopeFilter('all'); }} className="mt-4 rounded-lg px-3 py-2 text-sm font-medium text-zinc-700 underline decoration-zinc-300 underline-offset-4 transition hover:text-zinc-950 focus:outline-none focus:ring-2 focus:ring-zinc-950/20">Ver todos los paquetes</button>}</div>}</div>}
