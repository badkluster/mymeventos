...
import { OverduePaymentCollectionActions } from '@/components/admin/overdue-payment-contact';
...
const ticketOrderRef = (payment: Payment) => (typeof payment.ticketOrderId === 'string' ? undefined : payment.ticketOrderId);
...
export default function PaymentsPage() {
  ...
  const [filters, setFilters] = useState({ page: 1, status: '', type: '', method: '', source: '', search: '' });
  const query = useMemo(() => {
  ...
  return <section className="space-y-6">
    ...
    <div className="grid gap-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm lg:grid-cols-[1fr_160px_160px_160px_160px_auto]">
      ...
      <Select value={filters.source} onChange={(event) => updateFilters({ page: 1, source: event.target.value })}><option value="">Manuales y entradas</option><option value="manual">Solo manuales</option><option value="ticket_order">Solo entradas digitales</option></Select>
      <Button variant="secondary" onClick={applySearch}>Filtrar</Button>
    </div>

    <OverduePaymentCollectionActions payments={items} />
    <article className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
      {loading ? <p className="p-8 text-sm text-zinc-500">Cargando pagos...</p> : <div className="overflow-x-auto"><table className="min-w-[1320px] w-full text-sm"><thead className="border-b border-zinc-200 bg-zinc-50/80 text-zinc-500"><tr>{['Número', 'Fecha', 'Vencimiento', 'Cliente', 'Evento', 'Contrato', 'Salón', 'Tipo', 'Medio', 'Importe', 'Estado'].map((label) => <th key={label} className="whitespace-nowrap px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide">{label}</th>)}<th className="px-5 py-3.5 text-right text-xs font-semibold uppercase tracking-wide">Acciones</th></tr></thead><tbody className="divide-y divide-zinc-100">{items.map((payment) => { const ticketOrder = ticketOrderRef(payment); const isTicketOrder = payment.source === 'ticket_order'; return <tr key={payment._id} className="transition-colors hover:bg-amber-50/35"><td className="px-5 py-4 font-semibold text-zinc-900">{payment.paymentNumber}{isTicketOrder ? <span className="ml-2 inline-flex rounded-full bg-violet-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-violet-700">Entrada digital</span> : null}</td><td className="px-5 py-4 text-zinc-700">{formatDate(payment.paidAt ?? payment.createdAt)}</td><td className="px-5 py-4 text-zinc-700">{formatDate(payment.dueDate)}</td><td className="px-5 py-4 text-zinc-700">{isTicketOrder ? (ticketOrder?.buyer?.name || 'Comprador sin datos') : entityName(payment.customerId)}</td><td className="px-5 py-4 text-zinc-700">{isTicketOrder ? (ticketOrder ? <Link className="font-medium text-zinc-950 underline" href={`/admin/digital-tickets/orders/${ticketOrder._id}`}>Orden {ticketOrder.publicId}</Link> : 'Sin datos') : entityName(payment.eventId)}</td><td className="px-5 py-4">{isTicketOrder ? '—' : (entityId(payment.contractId) ? <Link className="font-medium text-zinc-950 underline" href={`/admin/contracts/${entityId(payment.contractId)}?tab=pagos`}>{entityName(payment.contractId)}</Link> : entityName(payment.contractId))}</td><td className="px-5 py-4 text-zinc-700">{isTicketOrder ? '—' : entityName(payment.salonId)}</td><td className="px-5 py-4 text-zinc-700">{displayLabel(paymentTypeLabels, payment.type)}</td><td className="px-5 py-4 text-zinc-700">{payment.method ? displayLabel(paymentMethodLabels, payment.method) : 'Pendiente'}</td><td className="px-5 py-4 font-medium text-zinc-900">{money(payment.amount)}</td><td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${statusTone[payment.status] ?? 'bg-zinc-100 text-zinc-700'}`}>{displayLabel(paymentStatusLabels, payment.status)}</span></td><td className="px-5 py-4"><div className="flex justify-end gap-0.5"><Link href={`/admin/payments/${payment._id}`}><TableActionButton icon={Eye} label="Ver pago" /></Link>{isTicketOrder ? null : <><TableActionButton icon={CheckCircle2} label="Marcar cobrado" disabled={savingId === payment._id || payment.status === 'paid'} onClick={() => void action(payment, 'mark-paid', 'Pago marcado como cobrado.')} /><TableActionButton icon={RotateCcw} label="Reembolsar" disabled={savingId === payment._id || payment.status !== 'paid' || payment.type === 'refund'} onClick={() => void action(payment, 'refund', 'Reembolso registrado.')} /><TableActionButton icon={XCircle} label="Cancelar" disabled={savingId === payment._id || payment.status === 'cancelled'} onClick={() => void action(payment, 'cancel', 'Pago cancelado.')} /></>}</div></td></tr>; })}</tbody></table>{items.length === 0 ? <p className="p-8 text-sm text-zinc-500">No hay pagos para los filtros seleccionados.</p> : null}</div>}
    </article>
