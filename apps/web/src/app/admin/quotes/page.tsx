...
export default function QuotesPage() {
  ...
  return <section className="space-y-6">
    ...
    {activeTab === 'requests' ? <RequestsTable requests={requests} salons={salons} loading={loading} onOpen={(request) => router.push(`/admin/quotes/requests/${request._id}`)} onTake={takeRequest} onConvert={openConvert} onWhatsApp={sendWhatsAppForRequest} onDuplicate={markDuplicated} onDiscard={setDiscard} /> : <QuotesTable quotes={quotes} salons={salons} loading={loading} onOpen={(quote) => router.push(`/admin/quotes/${quote._id}`)} onEdit={openEdit} onDuplicate={duplicate} onStatus={changeQuoteStatus} onRemove={setRemove} />}
