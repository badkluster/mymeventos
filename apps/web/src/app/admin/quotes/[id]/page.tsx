...
import { displayLabel, eventTypeLabels, quoteStatusLabels } from '@/lib/display-labels';
...
export default function QuoteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <section className="space-y-6 pb-8">
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <article className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm"><h2 className="text-base font-semibold text-zinc-950">Servicios incluidos</h2>{quote.includedServices?.length ? <ul className="mt-4 space-y-2 text-sm leading-6 text-zinc-600">{quote.includedServices.map((item) => <li key={item} className="flex gap-2"><Check className="mt-1 h-4 w-4 shrink-0 text-emerald-600" />{item}</li>)}</ul> : <p className="mt-4 text-sm text-zinc-500">No hay servicios cargados.</p>}<div className="mt-6 border-t border-zinc-100 pt-5"><h2 className="text-base font-semibold text-zinc-950">Consideraciones</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-zinc-600">{quote.considerations || 'Sin consideraciones.'}</p></div><div className="mt-6 border-t border-zinc-100 pt-5"><h2 className="text-base font-semibold text-zinc-950">Observaciones</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-zinc-600">{quote.observations || quote.notes || 'Sin observaciones.'}</p></div></article>
    </div>
...
function Item({ label, value }: { label: string; value: string | number }) { return <div><dt className="text-xs font-medium uppercase tracking-wide text-zinc-400">{label}</dt><dd className="mt-1 whitespace-pre-wrap break-words font-medium text-zinc-800">{value}</dd></div>; }
