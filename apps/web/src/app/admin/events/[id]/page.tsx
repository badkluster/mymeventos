  ...
  return <section className="space-y-6 pb-8">
    ...
    <header className="rounded-3xl border border-zinc-200 bg-white px-6 py-6 shadow-sm md:px-8">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
        <div><div className="flex flex-wrap items-center gap-3"><h1 className="text-3xl font-semibold tracking-tight text-zinc-950">{event.eventName || event.eventType || 'Evento'}</h1><span className="inline-flex rounded-full bg-zinc-100 px-2.5 py-1 text-xs font-semibold text-zinc-700">{displayLabel(eventStatusLabels, event.status)}</span></div><p className="mt-2 text-sm text-zinc-500">{entityName(event.customerId)} · {entityName(event.salonId)}</p><div className="mt-3 flex flex-wrap gap-2"><Link href={`/admin/events/${event._id}/invitations`} className="text-sm font-medium text-zinc-700 underline">Invitación digital</Link><Link href={`/admin/events/${event._id}/tickets`} className="text-sm font-medium text-zinc-700 underline">Entradas</Link><Link href={`/admin/events/${event._id}/check-in`} className="text-sm font-medium text-zinc-700 underline">Control de ingreso</Link></div></div>
