...
import { activityTypeLabels, displayLabel, eventStaffStatusLabels, eventStatusLabels, paymentMethodLabels, paymentTypeLabels, quoteStatusLabels, staffSubroleLabels } from '@/lib/display-labels';
...
export default function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const registerPayment = async () => {
    ...
    setSaving(true);
    try {
      const response = await api.post<{ planOverpaymentAmount?: number }>(`/events/${event._id}/payments`, { ...newPayment, amount: Number(newPayment.amount), planInstallmentId: newPayment.planInstallmentId || undefined });
  ...
  return <section className="space-y-6 pb-8">
    ...
    {activeTab === 'pagos' && <OverdueInstallmentCollectionActions eventId={event._id} installments={event.paymentPlanSnapshot?.length ? event.paymentPlanSnapshot : contract?.paymentPlanSnapshot ?? []} />}
