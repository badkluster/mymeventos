...
import { CalendarPlus, ChevronLeft, CreditCard, Download, Eye, FileText, LockKeyhole, Mail, MessageCircle, Plus, Printer, Trash2 } from 'lucide-react';
...
export default function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const registerPayment = async () => {
    ...
    setSaving(true);
    try {
      const response = await api.post<{ items: Payment[]; summary: PaymentSummary; contract?: Contract; paymentPlanSnapshot?: Event['paymentPlanSnapshot']; planOverpaymentAmount?: number }>(`/events/${event._id}/payments`, { ...newPayment, amount: Number(newPayment.amount), planInstallmentId: newPayment.planInstallmentId || undefined });
