...
export default function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [editForm, setEditForm] = useState({ fullName: '', phone: '', email: '', documentNumber: '', address: '', occupation: '', notes: '' });
  ...
  const openEdit = () => { setEditForm({ fullName: customerName(customer), phone: customer.phone ?? '', email: customer.email ?? '', documentNumber: customer.documentNumber ?? '', address: customer.address ?? '', occupation: customer.occupation ?? '', notes: customer.notes ?? '' }); setEditOpen(true); };
