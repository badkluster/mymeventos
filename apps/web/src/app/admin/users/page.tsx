...
type User = {
  _id: string; username?: string; email?: string; firstName?: string; lastName?: string; fullName?: string; avatarUrl?: string; phone?: string; documentType?: string; documentNumber?: string; roles?: string[];
...
const initials = (user: User) => {
  const names = (user.fullName || [user.firstName, user.lastName].filter(Boolean).join(' ') || user.username || user.email || 'U').trim().split(/\s+/);
  return names.slice(0, 2).map((item) => item[0]).join('').toUpperCase();
};
...
function UserModal({ modal, form, setForm, roles, salons, saving, editing, onClose, onSave, onDelete }: { modal: string | null; form: typeof emptyForm; setForm: Dispatch<SetStateAction<typeof emptyForm>>; roles: string[]; salons: Salon[]; saving: boolean; editing: User | null; onClose: () => void; onSave: () => void; onDelete: () => void }) {
  ...
  const canSubmit = Boolean(form.roles.length && (modal !== 'create' || (form.firstName.trim() && form.lastName.trim() && form.username.trim().length >= 3 && form.email.trim() && form.password.length >= 8)));
