...
type User = {
  ...
  permissionOverrides?: string[]; permissionDeniedOverrides?: string[]; active?: boolean; lastLoginAt?: string; employeeProfile?: { position?: string };
};
...
const emptyForm = { firstName: '', lastName: '', username: '', email: '', password: '', phone: '', documentType: 'DNI', documentNumber: '', roles: ['STAFF'], salonIds: [] as string[], managedSalonIds: [] as string[], primarySalonId: '', primaryManagedSalonId: '', active: true, position: '' };
...
export default function UsersPage() {
  ...
  return <section className="space-y-6">
    ...
    <div className="grid gap-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm lg:grid-cols-[1fr_220px_180px_auto]">
    ...
    <article className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
      {loading ? <p className="p-8 text-sm text-zinc-500">Cargando usuarios...</p> : <div className="overflow-x-auto"><table className="min-w-[1240px] w-full text-sm"><thead className="border-b border-zinc-200 bg-zinc-50/80 text-zinc-500"><tr>{['Usuario', 'Email', 'Teléfono', 'Roles', 'Salones', 'Salones a cargo', 'Estado', 'Último acceso'].map((label) => <th key={label} className="whitespace-nowrap px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide">{label}</th>)}<th className="px-5 py-3.5 text-right text-xs font-semibold uppercase tracking-wide">Acciones</th></tr></thead><tbody className="divide-y divide-zinc-100">{items.map((user) => <tr key={user._id} className="transition-colors hover:bg-amber-50/35"><td className="px-5 py-4"><p className="font-medium text-zinc-900">{name(user)}</p><p className="mt-1 text-xs text-zinc-500">{user.username}</p></td><td className="px-5 py-4 text-zinc-700">{user.email || 'No informado'}</td><td className="px-5 py-4 text-zinc-700">{user.phone || 'No informado'}</td><td className="px-5 py-4 text-zinc-700">{user.roles?.map((role) => displayLabel(roleLabels, role)).join(', ') || 'Sin rol'}</td><td className="px-5 py-4 text-zinc-700">{salonLabel(user.salonIds)}</td><td className="px-5 py-4 text-zinc-700">{salonLabel(user.managedSalonIds)}</td><td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${user.active === false ? 'bg-zinc-100 text-zinc-700' : 'bg-emerald-50 text-emerald-700'}`}>{user.active === false ? 'Inactivo' : 'Activo'}</span></td><td className="px-5 py-4 text-zinc-700">{formatDate(user.lastLoginAt)}</td><td className="px-5 py-4"><div className="flex justify-end gap-0.5"><Link href={`/admin/users/${user._id}`}><TableActionButton icon={Eye} label="Ver detalle" /></Link>{canUpdate ? <TableActionButton icon={Pencil} label="Editar usuario" onClick={() => openEdit(user, 'edit')} /> : null}{isAdmin ? <TableActionButton icon={UserCog} label="Roles" onClick={() => openEdit(user, 'roles')} /> : null}{canUpdate ? <TableActionButton icon={ShieldCheck} label="Salones" onClick={() => openEdit(user, 'salons')} /> : null}{canUpdate ? <TableActionButton icon={user.active === false ? ToggleRight : ToggleLeft} label={user.active === false ? 'Activar' : 'Desactivar'} disabled={savingId === user._id} onClick={() => void toggleActive(user)} /> : null}{canDelete ? <TableActionButton icon={Trash2} label="Eliminar" disabled={savingId === user._id} onClick={() => openEdit(user, 'delete')} /> : null}</div></td></tr>)}</tbody></table>{items.length === 0 ? <Empty /> : null}</div>}
    </article>
