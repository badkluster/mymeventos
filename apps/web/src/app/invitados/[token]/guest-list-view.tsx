      ...
      try {
        ...
        if (!savedWhileLoading.current && isGuestListReadOnly(response.access)) {
          ...
          if (latestDraft) {
            ...
            if (localDraftConflictsWithServer(latestDraft, response.guestList)) setNotice('La lista del servidor cambió desde que empezaste este borrador. Tus cambios siguen en este dispositivo, pero no se enviarán hasta que revises la versión actual con M&M Eventos.');
          } else {
            setGuestList(normalizeGuestList(response.guestList, response.event.guestCount)); setDirty(false); setHasLocalDraft(false);
          }
        }
