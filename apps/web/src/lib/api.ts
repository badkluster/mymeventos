...
type ApiErrorField = { path?: string; message?: string };
type ApiErrorDetails = { fields?: ApiErrorField[]; [key: string]: unknown };
...
function detailedErrorMessage(error?: ApiErrorPayload, fallback = 'No se pudo completar la solicitud.'): string {
  ...
  if (uniqueMessages.length > 1) return `Revisá los datos: ${uniqueMessages.join(' · ')}`;
...
async function rawRequest<T>(path: string, init?: RequestInit): Promise<{ response: Response; payload: ApiEnvelope<T> }> {
  ...
  const payload = await response.json().catch(() => ({
    success: false,
    error: { code: 'NETWORK_ERROR', message: 'No se pudo completar la solicitud.' },
  })) as ApiEnvelope<T>;
  return { response, payload };
}

async function refreshSessionOnce(): Promise<void> {
  refreshPromise ??= rawRequest('/auth/refresh', { method: 'POST', body: JSON.stringify({}) }).finally(() => {
    refreshPromise = null;
  });
...
async function request<T>(path: string, init?: RequestInit, retried = false): Promise<T> {
  ...
  if (!response.ok || !payload.success) {
    throw new ApiClientError(
      ...
      detailedErrorMessage(payload.error),
...
async function download(path: string, retried = false): Promise<{ blob: Blob; filename: string }> {
  ...
  if (!response.ok) {
    ...
    throw new ApiClientError(
      ...
      detailedErrorMessage(payload?.error, 'No se pudo generar el archivo.'),
      payload?.error?.details,
    );
  }
