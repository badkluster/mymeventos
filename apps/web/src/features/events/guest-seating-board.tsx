...
function ageGroupLabel(ageGroup?: string): string {
  return ({ child_1_4: '1 a 4 años · sin cargo', child_5_9: '5 a 9 años · media tarifa', minor_10_17: '10 a 17 años · menor' } as Record<string, string>)[ageGroup ?? ''] ?? '';
}
...
function guestDetail(guest: EventGuest): string {
  ...
  return [ageGroupLabel(guest.ageGroup), guest.meal, restriction].filter(Boolean).join(' · ') || 'Sin menú definido';
}

function GuestChip({ guest, onDragStart, onDragEnd }: { guest: EventGuest; onDragStart: (event: DragEvent<HTMLDivElement>) => void; onDragEnd: () => void }) {
  return <div draggable onDragStart={onDragStart} onDragEnd={onDragEnd} className="group flex min-w-0 cursor-grab items-center gap-2 rounded-lg border border-zinc-200 bg-white px-2 py-1.5 text-xs text-zinc-700 shadow-sm transition hover:border-amber-300 hover:bg-amber-50 active:cursor-grabbing"><GripVertical className="h-3.5 w-3.5 shrink-0 text-zinc-300 group-hover:text-amber-500" /><div className="min-w-0"><p className="truncate font-medium text-zinc-950">{guest.fullName || 'Invitado sin nombre'}</p><p className="truncate text-[11px] text-zinc-500">{guestDetail(guest)}</p></div></div>;
}

export function GuestSeatingBoard({ tables, guests, onAssign, showUnassigned = true, onEditTable, onViewTable, onAddGuestToTable }: GuestSeatingBoardProps) {
