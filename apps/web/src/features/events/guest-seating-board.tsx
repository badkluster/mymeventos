...
function GuestChip({ guest, onDragStart, onDragEnd }: { guest: EventGuest; onDragStart: (event: DragEvent<HTMLDivElement>) => void; onDragEnd: () => void }) {
  ...
  return <div draggable onDragStart={onDragStart} onDragEnd={onDragEnd} className={`group flex min-w-0 cursor-grab items-center gap-2 rounded-lg border px-2 py-1.5 text-xs shadow-sm transition active:cursor-grabbing ${ageMeta.chipClassName}`}><GripVertical className="h-3.5 w-3.5 shrink-0 text-zinc-400 group-hover:text-zinc-700" /><div className="min-w-0"><p className="truncate font-medium text-zinc-950">{guest.fullName || 'Invitado sin nombre'}</p><p className="truncate text-[11px] text-zinc-600">{guestDetail(guest)}</p></div></div>;
}
