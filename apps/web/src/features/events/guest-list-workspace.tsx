...
import { AlertTriangle, CheckCircle2, Eye, FileText, Plus, RotateCcw, Save, Settings, TableProperties, UserPlus, Users } from 'lucide-react';
...
function withIds(list?: EventGuestList): EventGuestList {
  return {
    ...emptyGuestList,
    ...list,
    tables: (list?.tables ?? []).map((table) => ({ ...table, id: table.id || makeId() })),
...
function dateLabel(value?: string): string {
  ...
  return Number.isNaN(date.getTime()) ? 'Fecha a confirmar' : new Intl.DateTimeFormat('es-AR', { dateStyle: 'long' }).format(date);
}
...
export function GuestListWorkspace({ event, plan, saving, onSave, onSyncSummary, onNotice }: GuestListWorkspaceProps) {
  const initialGuestList = useMemo(() => withIds(plan?.guestList), [plan]);
  ...
  const shareUrl = shareToken ? guestListUrl(shareToken) : '';
  ...
  const discard = () => { setGuestList(withIds(plan?.guestList)); setDirty(false); setSelectedGuestIds([]); setTableFilterId(undefined); };
  ...
  return <div className="mx-auto w-full max-w-[1440px] space-y-5 rounded-3xl bg-zinc-50/80 p-1 sm:p-3"><header className="overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-950 px-5 py-6 text-white shadow-lg sm:px-7"><div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between"><div className="flex min-w-0 items-start gap-4"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl border border-amber-300/50 bg-amber-300/10 font-serif text-xl font-semibold text-amber-300">M</div><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[.16em] text-amber-300">M&M Eventos</p><h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">Lista de invitados</h1><p className="mt-1 truncate text-sm text-zinc-300">{event.eventType || 'Evento'} · {customerName(event)} · {dateLabel(event.eventDate)}</p></div></div><div className="flex flex-col gap-3 xl:items-end"><p className={`inline-flex items-center gap-2 text-sm ${saving ? 'text-amber-200' : dirty ? 'text-amber-200' : 'text-emerald-300'}`}><CheckCircle2 className="h-4 w-4" />{status}</p><div className="flex flex-wrap gap-2"><Button type="button" variant="secondary" onClick={() => setTab('summary')}><Eye className="mr-2 h-4 w-4" />Vista previa</Button><Button type="button" disabled={saving || !dirty} onClick={save}><Save className="mr-2 h-4 w-4" />{saving ? 'Guardando...' : 'Guardar cambios'}</Button></div></div></div><div className="mt-6 flex gap-3 overflow-x-auto pb-1"><Metric value={validGuests.length} label="invitados" icon={<Users className="h-4 w-4" />} /><Metric value={assignedGuests.length} label="asignados" icon={<CheckCircle2 className="h-4 w-4" />} /><Metric value={unassignedGuests.length} label="sin mesa" icon={<UserPlus className="h-4 w-4" />} /><Metric value={tables.length} label="mesas" icon={<TableProperties className="h-4 w-4" />} /></div></header><GuestListTabs value={tab} onChange={setTab} />
    ...
    {tab === 'guests' ? <div className="space-y-5"><section className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm"><div><h2 className="font-semibold text-zinc-950">Gestión de invitados</h2><p className="mt-1 text-sm text-zinc-500">Podés cargar invitados individualmente, en lote o compartir el formulario con el cliente.</p></div><div className="flex flex-wrap gap-2"><Button type="button" variant="secondary" onClick={() => setQuickImportOpen(true)}><Plus className="mr-2 h-4 w-4" />Carga rápida</Button><Button type="button" variant="secondary" disabled={sharing} onClick={() => void createShareLink()}>{sharing ? 'Preparando enlace...' : shareUrl ? 'Copiar enlace cliente' : 'Crear enlace cliente'}</Button><Button type="button" onClick={() => openGuestEditor()}><UserPlus className="mr-2 h-4 w-4" />Agregar invitado</Button></div>{shareUrl ? <div className="w-full rounded-xl border border-sky-200 bg-sky-50 p-3 text-sm text-sky-950"><p className="font-medium">Enlace permanente del cliente</p><p className="mt-1 text-sky-800">Este mismo enlace seguirá activo aunque vuelvas a editar la lista.</p><div className="mt-2 flex flex-col gap-2 sm:flex-row"><Input readOnly value={shareUrl} /><a className="inline-flex shrink-0 items-center justify-center rounded-xl bg-white px-4 py-2.5 font-medium shadow-sm ring-1 ring-sky-200" href={shareUrl} target="_blank" rel="noreferrer">Abrir formulario</a></div></div> : null}</section><GuestDirectory guests={guests} tables={tables} selectedGuestIds={selectedGuestIds} tableFilterId={tableFilterId} onToggleSelected={toggleGuest} onClearSelected={() => setSelectedGuestIds([])} onBulkAssign={bulkAssign} onEditGuest={(guestId) => openGuestEditor(guestId)} onDeleteGuest={deleteGuest} onClearTableFilter={() => setTableFilterId(undefined)} /></div> : null}
