...
const operationalViews = [
  ...
  ['logistics', 'Logística'],
  ['linen', 'Vajilla y stock'],
  ['products', 'Stock de productos']
] as const;

export function EventOperationsWorkspace({ event, plan, saving, onSave, onSaveGuestList, onSyncSummary, onNotice }: { event: Event; plan?: EventResourcePlan; saving: boolean; onSave: SavePlan; onSaveGuestList: (guestList: EventGuestList) => Promise<EventGuestList | undefined>; onSyncSummary: (payload: Record<string, unknown>) => void; onNotice?: (message: string, variant?: 'success' | 'error') => void }) {
  ...
  return <div className="space-y-5"><div className="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm"><div className="flex gap-2 overflow-x-auto"><div className="flex min-w-max gap-2">{operationalViews.map(([value, label]) => <button key={value} type="button" onClick={() => setView(value)} className={`rounded-xl px-4 py-2.5 text-sm font-medium transition ${view === value ? 'bg-zinc-950 text-white shadow-sm' : 'bg-zinc-50 text-zinc-600 hover:bg-zinc-100 hover:text-zinc-950'}`}>{label}</button>)}</div></div></div>
    ...
    {view === 'guests' && <EventGuestListEditor event={event} plan={plan} saving={saving} onSaveGuestList={onSaveGuestList} onSyncSummary={onSyncSummary} onNotice={onNotice} />}
...
export function EventGuestListEditor({ event, plan, saving, onSaveGuestList, onSyncSummary, onNotice }: { event: Event; plan?: EventResourcePlan; saving: boolean; onSaveGuestList: (guestList: EventGuestList) => Promise<EventGuestList | undefined>; onSyncSummary: (payload: Record<string, unknown>) => void; onNotice?: (message: string, variant?: 'success' | 'error') => void }) {
  return <GuestListWorkspace event={event} plan={plan} saving={saving} onSaveGuestList={onSaveGuestList} onSyncSummary={onSyncSummary} onNotice={onNotice} />;
}
  ...
  return <div className="space-y-5">
    {section !== 'inventory' && <SectionCard title="Stock de productos" icon={<PackageCheck className="h-4 w-4" />} action={<Button variant="secondary" onClick={() => setProducts((current) => [...current, { id: makeId(), name: '', category: '', productionCategory: productCategory === 'all' ? 'savory' : productCategory, quantity: undefined, unit: 'unidad', status: 'planned', notes: '' }])}><Plus className="mr-2 h-4 w-4" />Agregar insumo</Button>}>
      ...
      {visibleProducts.length ? <div className="space-y-3">{visibleProducts.map(({ item, index }) => <article key={item.id ?? index} className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm transition hover:border-zinc-300">
        ...
        <details className="mt-4 rounded-xl bg-zinc-50 px-3 py-2.5" open={Boolean(item.notes)}><summary className="cursor-pointer text-sm font-medium text-zinc-600 marker:text-zinc-400">Notas, entrega o reposición</summary><Textarea aria-label="Notas" className="mt-3 min-h-20 bg-white" placeholder="Marca, horario de entrega, compra pendiente, reposición..." value={item.notes ?? ''} onChange={(event) => updateProduct(index, { notes: event.target.value })} /></details>
  ...
  return <><SectionCard title="Servicios externos y proveedores" icon={<Truck className="h-4 w-4" />} action={<div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={() => setQuickOpen(true)}><Plus className="mr-2 h-4 w-4" />Alta rápida</Button><Button variant="secondary" onClick={() => setItems((current) => [...current, { id: makeId(), supplierId: '', supplierName: '', serviceType: '', status: 'pending', notes: '' }])}><Plus className="mr-2 h-4 w-4" />Asignar proveedor</Button></div>}>
    ...
    {items.length ? <div className="space-y-3">{items.map((item, index) => {
      ...
      return <article key={item.id ?? index} className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm transition hover:border-zinc-300">
        ...
        <details className="mt-4 rounded-xl bg-zinc-50 px-3 py-2.5" open={Boolean(item.notes)}><summary className="cursor-pointer text-sm font-medium text-zinc-600 marker:text-zinc-400">Notas, condiciones y seguimiento</summary><Textarea aria-label="Notas" className="mt-3 min-h-20 bg-white" placeholder="Horarios, condiciones de pago, requerimientos y observaciones..." value={item.notes ?? ''} onChange={(event) => update(index, { notes: event.target.value })} /></details>
  ...
  return <div className="space-y-5">
    ...
    <SectionCard title="Alertas y recordatorios" icon={<CalendarClock className="h-4 w-4" />} action={<Button variant="secondary" onClick={() => setAlerts((current) => [...current, { id: makeId(), title: '', remindAt: '', channel: 'system', status: 'pending', notes: '' }])}><Plus className="mr-2 h-4 w-4" />Agregar alerta</Button>}>
      {alerts.length ? <div className="space-y-3">{alerts.map((item, index) => <div key={item.id ?? index} className="grid gap-3 rounded-xl border border-zinc-200 bg-zinc-50/70 p-3 lg:grid-cols-[minmax(180px,1fr)_170px_130px_130px_44px]">
        ...
        <Select aria-label="Canal" value={item.channel === 'email' ? 'email' : 'system'} onChange={(event) => updateAlert(index, { channel: event.target.value })}><option value="system">Sistema</option><option value="email">Email</option></Select>
