...
import { ArrowLeft, Ban, Check, CheckCheck, LockKeyhole, Pencil, Plus, RotateCcw } from 'lucide-react';
...
type Plan = {
  _id: string; status: string; eventDate: string; notes?: string; guestCounts: { total?: number };
  eventId?: { eventName?: string; eventType?: string; startTime?: string }; customerId?: { fullName?: string };
...
const statusTone: Record<string, string> = { pending: 'bg-amber-50 text-amber-800', in_progress: 'bg-sky-50 text-sky-700', ready: 'bg-violet-50 text-violet-700', checked: 'bg-emerald-50 text-emerald-700', blocked: 'bg-red-50 text-red-700', cancelled: 'bg-zinc-100 text-zinc-500' };
...
const date = new Intl.DateTimeFormat('es-AR', { timeZone: 'America/Argentina/Buenos_Aires', dateStyle: 'long' });
...
export function ProductionDetail({ planId }: { planId: string }) {
  ...
  const load = useCallback(async () => {
    try { setPlan((await api.get<{ plan: Plan }>(`/production/plans/${planId}`)).plan); }
  ...
  return <section className="space-y-5">
    ...
    <PageHeader title={plan.eventId?.eventName || plan.eventId?.eventType || 'Producción del evento'} description={`${date.format(new Date(plan.eventDate))}${plan.eventId?.startTime ? ` · ${plan.eventId.startTime}` : ''} · ${plan.salonId?.name || 'Sin salón'}`} action={<div className="flex gap-2">{plan.status === 'closed' && canReopen ? <Button variant="secondary" onClick={() => setReopenOpen(true)}><RotateCcw className="mr-2 h-4 w-4" />Reabrir</Button> : null}{!locked && canComplete ? <Button variant="secondary" disabled={savingId === 'close'} onClick={() => void close()}><LockKeyhole className="mr-2 h-4 w-4" />Cerrar producción</Button> : null}{!locked && canCreate ? <Button onClick={() => setForm({ ...blankForm })}><Plus className="mr-2 h-4 w-4" />Agregar ítem</Button> : null}</div>} />
    ...
    {!allItems.length ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-800">La producción se generó sin ítems porque el evento no tenía productos legados y todavía no hay reglas aplicables. Agregá ítems manuales o configurá reglas.</div> : null}
