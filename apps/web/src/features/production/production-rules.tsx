...
type Rule = {
  ...
  quantityPerGuest: number; fixedQuantity: number; roundingMode: string; wastePercentage: number; sectionType: string; isActive: boolean; notes?: string;
};
...
export function ProductionRules() {
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api.post('/production/rules', {
        ...
        isActive: true, notes: String(data.get('notes') || ''),
      });
      setOpen(false); showToast({ message: 'Regla creada correctamente.', variant: 'success' }); await load();
    } catch (cause) { showToast({ message: cause instanceof Error ? cause.message : 'No se pudo crear la regla.', variant: 'error' }); }
  ...
  return <section className="space-y-5">
    <PageHeader title="Reglas de producción" description="Transforman invitados y condiciones del evento en cantidades reproducibles. Cada generación conserva su snapshot." action={<Button onClick={() => setOpen(true)}><Plus className="mr-2 h-4 w-4" />Nueva regla</Button>} />
    <ProductionNav />
    <article className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="min-w-[1000px] w-full text-sm"><thead className="border-b border-zinc-200 bg-zinc-50/80"><tr>{['Regla', 'Producto', 'Sección', 'Tipo de evento', 'Rango invitados', 'Cálculo', 'Merma', 'Estado', ''].map((label) => <th key={label} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500">{label}</th>)}</tr></thead><tbody className="divide-y divide-zinc-100">{items.map((rule) => <tr key={rule._id}><td className="px-4 py-3 font-semibold">{rule.name}</td><td className="px-4 py-3">{rule.productId?.name || 'Producto no disponible'}</td><td className="px-4 py-3">{sectionLabels[rule.sectionType]}</td><td className="px-4 py-3">{rule.eventType || 'Todos'}</td><td className="px-4 py-3">{rule.guestsFrom ?? 0} — {rule.guestsTo ?? 'sin máximo'}</td><td className="px-4 py-3">{rule.fixedQuantity || 0} + {rule.quantityPerGuest || 0} por invitado · {roundingLabels[rule.roundingMode]}</td><td className="px-4 py-3">{rule.wastePercentage || 0}%</td><td className="px-4 py-3"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${rule.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-zinc-100 text-zinc-500'}`}>{rule.isActive ? 'Activa' : 'Inactiva'}</span></td><td className="px-4 py-3"><button title={rule.isActive ? 'Desactivar' : 'Activar'} className="grid h-8 w-8 place-items-center rounded-lg hover:bg-zinc-100" onClick={() => void toggle(rule)}><Power className="h-4 w-4" /></button></td></tr>)}</tbody></table>{!items.length ? <div className="grid min-h-52 place-items-center text-sm text-zinc-500">Todavía no hay reglas de producción.</div> : null}</div></article>
    <Modal open={open} onClose={() => setOpen(false)} title="Nueva regla de producción" description="La cantidad final se calcula en backend y queda registrada en el snapshot del plan.">
      <form onSubmit={create} className="grid gap-4 p-6 sm:grid-cols-2"><label className="text-sm font-medium">Nombre<Input name="name" required className="mt-1.5" placeholder="Ej. Empanadas por invitado" /></label><label className="text-sm font-medium">Producto<Select name="productId" required className="mt-1.5"><option value="">Seleccionar…</option>{products.map((product) => <option key={product._id} value={product._id}>{product.name} · {product.unitOfMeasure}</option>)}</Select></label><label className="text-sm font-medium">Sección<Select name="sectionType" className="mt-1.5">{Object.entries(sectionLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select></label><label className="text-sm font-medium">Tipo de evento<Input name="eventType" className="mt-1.5" placeholder="Vacío: todos" /></label><label className="text-sm font-medium">Invitados desde<Input name="guestsFrom" type="number" min="0" className="mt-1.5" /></label><label className="text-sm font-medium">Invitados hasta<Input name="guestsTo" type="number" min="0" className="mt-1.5" /></label><label className="text-sm font-medium">Cantidad fija<Input name="fixedQuantity" type="number" min="0" step="0.001" defaultValue="0" className="mt-1.5" /></label><label className="text-sm font-medium">Cantidad por invitado<Input name="quantityPerGuest" type="number" min="0" step="0.001" defaultValue="0" className="mt-1.5" /></label><label className="text-sm font-medium">Redondeo<Select name="roundingMode" className="mt-1.5">{Object.entries(roundingLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select></label><label className="text-sm font-medium">Tamaño de empaque<Input name="packageSize" type="number" min="0" step="0.001" className="mt-1.5" /></label><label className="text-sm font-medium">Merma %<Input name="wastePercentage" type="number" min="0" max="100" defaultValue="0" className="mt-1.5" /></label><label className="sm:col-span-2 text-sm font-medium">Notas<Textarea name="notes" className="mt-1.5" /></label><footer className="sm:col-span-2 flex justify-end gap-3"><Button type="button" variant="secondary" onClick={() => setOpen(false)}>Cancelar</Button><Button disabled={saving}>{saving ? 'Guardando…' : 'Crear regla'}</Button></footer></form>
    </Modal>
  </section>;
}
