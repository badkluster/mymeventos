...
type IssuedTicket = { _id: string; ticketTypeId?: { name?: string }; attendeeName?: string; status?: string };
type Refund = { _id: string; amount: number; status: string };
...
export function TicketOrderDetail({ orderId }: { orderId: string }) {
  ...
  const load = () => api.get<TicketOrderDetails>(`/tickets/orders/${orderId}`).then(setData).catch((error: unknown) => showToast({ message: error instanceof Error ? error.message : 'No se pudo cargar la orden.', variant: 'error' }));
