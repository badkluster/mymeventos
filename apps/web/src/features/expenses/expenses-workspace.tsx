...
type Expense = {
  ...
  paymentMethod?: string; receiptFileId?: string; receiptUrl?: string; notes?: string;
};
type Receipt = { fileId?: string; url: string };
...
export function ExpensesWorkspace() {
  ...
  const [receipt, setReceipt] = useState<Receipt>({ url: '' });
  ...
  const openEditor = (expense: Expense | null) => {
    setReceipt({ fileId: expense?.receiptFileId, url: expense?.receiptUrl ?? '' });
    setForm(expense);
  };
  const useUploadedReceipt = (asset: UploadedAsset) => setReceipt({ fileId: asset.publicId, url: asset.secureUrl || asset.url });

  const save = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setSaving(true); const data = new FormData(event.currentTarget);
    const payload = { date: String(data.get('date')), description: String(data.get('description')), salonId: String(data.get('salonId')), eventId: String(data.get('eventId')), supplierId: String(data.get('supplierId')), categoryId: String(data.get('categoryId')), initialEstimatedAmount: Number(data.get('initialEstimatedAmount') || 0), finalAmount: Number(data.get('finalAmount') || 0), additionalAmount: Number(data.get('additionalAmount') || 0), taxAmount: Number(data.get('taxAmount') || 0), status: String(data.get('status')), paymentMethod: String(data.get('paymentMethod')), receiptFileId: receipt.fileId ?? '', receiptUrl: receipt.url, notes: String(data.get('notes')) };
    try {
  ...
  return <section className="space-y-5">
    <PageHeader title="Gastos y costos" description="Registro estructurado de costos estimados, finales, adicionales e impuestos." action={<div className="flex gap-2">{canCategories ? <Button variant="secondary" onClick={() => setCategoryOpen(true)}>Nueva categoría</Button> : null}{canCreate ? <Button onClick={() => openEditor(null)}><Plus className="mr-2 h-4 w-4" />Registrar gasto</Button> : null}</div>} />
    <ExpensesNav />
    ...
    <Modal open={form !== undefined} onClose={() => setForm(undefined)} title={form ? 'Editar gasto' : 'Registrar gasto'} description="El total se calcula como final + adicional + impuestos. El estimado se conserva para medir desvíos.">
      <form onSubmit={save} className="grid gap-4 p-6 sm:grid-cols-2 lg:grid-cols-3"><label className="text-sm font-medium">Fecha<Input name="date" type="date" required defaultValue={form?.date ? String(form.date).slice(0, 10) : defaultDate} className="mt-1.5" /></label><label className="sm:col-span-2 text-sm font-medium">Concepto<Input name="description" required defaultValue={form?.description} className="mt-1.5" /></label><label className="text-sm font-medium">Salón<Select name="salonId" required defaultValue={id(form?.salonId)} className="mt-1.5"><option value="">Seleccionar…</option>{salons.map((item) => <option key={item._id} value={item._id}>{entity(item)}</option>)}</Select></label><label className="text-sm font-medium">Evento<Select name="eventId" defaultValue={id(form?.eventId)} className="mt-1.5"><option value="">Gasto general</option>{events.map((item) => <option key={item._id} value={item._id}>{entity(item)}</option>)}</Select></label><label className="text-sm font-medium">Proveedor<Select name="supplierId" defaultValue={id(form?.supplierId)} className="mt-1.5"><option value="">Sin proveedor</option>{suppliers.map((item) => <option key={item._id} value={item._id}>{entity(item)}</option>)}</Select></label><label className="text-sm font-medium">Categoría<Select name="categoryId" defaultValue={id(form?.categoryId)} className="mt-1.5"><option value="">Sin categoría</option>{categories.filter((item) => item.isActive).map((item) => <option key={item._id} value={item._id}>{item.name}</option>)}</Select></label><label className="text-sm font-medium">Estimado inicial<Input name="initialEstimatedAmount" type="number" min="0" step="0.01" defaultValue={form?.initialEstimatedAmount ?? 0} className="mt-1.5" /></label><label className="text-sm font-medium">Importe final<Input name="finalAmount" type="number" min="0" step="0.01" defaultValue={form?.finalAmount ?? 0} className="mt-1.5" /></label><label className="text-sm font-medium">Adicional<Input name="additionalAmount" type="number" min="0" step="0.01" defaultValue={form?.additionalAmount ?? 0} className="mt-1.5" /></label><label className="text-sm font-medium">Impuestos<Input name="taxAmount" type="number" min="0" step="0.01" defaultValue={form?.taxAmount ?? 0} className="mt-1.5" /></label><label className="text-sm font-medium">Estado<Select name="status" defaultValue={form?.status ?? 'pending'} className="mt-1.5">{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select></label><label className="text-sm font-medium">Medio de pago<Select name="paymentMethod" defaultValue={form?.paymentMethod ?? ''} className="mt-1.5"><option value="">Sin informar</option>{Object.entries(paymentLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select></label><div className="sm:col-span-2 lg:col-span-3 rounded-xl border border-dashed border-zinc-300 bg-zinc-50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium text-zinc-900">Ticket o comprobante</p><p className="mt-1 text-xs leading-5 text-zinc-500">Adjuntá una imagen o PDF para respaldar este gasto. Máximo 25 MB.</p></div><CloudinaryUpload context="expenses" accept="image/jpeg,image/png,image/webp,image/avif,image/gif,application/pdf" label={receipt.url ? 'Reemplazar archivo' : 'Adjuntar ticket'} onUploaded={useUploadedReceipt} /></div>{receipt.url ? <div className="mt-3 flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-1.5 text-sm font-medium text-emerald-700"><FileText className="h-4 w-4" />Comprobante adjunto</span><a href={receipt.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm font-medium underline underline-offset-2">Abrir <ExternalLink className="h-3.5 w-3.5" /></a><button type="button" onClick={() => setReceipt({ url: '' })} className="text-sm font-medium text-zinc-600 underline underline-offset-2 hover:text-zinc-950">Quitar adjunto</button></div> : null}<label className="mt-3 block text-sm font-medium">URL externa del comprobante<Input type="url" value={receipt.url} onChange={(event) => setReceipt({ url: event.target.value })} className="mt-1.5" placeholder="https://…" /></label></div><label className="sm:col-span-2 lg:col-span-3 text-sm font-medium">Notas<Textarea name="notes" defaultValue={form?.notes} className="mt-1.5" /></label><footer className="sm:col-span-2 lg:col-span-3 flex justify-end gap-3"><Button type="button" variant="secondary" onClick={() => setForm(undefined)}>Cancelar</Button><Button disabled={saving}>{saving ? 'Guardando…' : 'Guardar gasto'}</Button></footer></form>
    </Modal>
    <Modal open={Boolean(detail)} onClose={() => setDetail(null)} title="Detalle del gasto" description={detail ? `${detail.description} · ${date.format(new Date(detail.date || detail._id))}` : ''}>
      {detail ? <div className="space-y-5 p-6"><dl className="grid gap-4 text-sm sm:grid-cols-2"><div><dt className="text-zinc-500">Salón</dt><dd className="mt-1 font-medium text-zinc-950">{entity(detail.salonId)}</dd></div><div><dt className="text-zinc-500">Evento</dt><dd className="mt-1 font-medium text-zinc-950">{entity(detail.eventId)}</dd></div><div><dt className="text-zinc-500">Proveedor</dt><dd className="mt-1 font-medium text-zinc-950">{entity(detail.supplierId)}</dd></div><div><dt className="text-zinc-500">Categoría</dt><dd className="mt-1 font-medium text-zinc-950">{detail.categoryId?.name || 'Sin categoría'}</dd></div><div><dt className="text-zinc-500">Total</dt><dd className="mt-1 font-semibold text-zinc-950">{money.format(detail.amount)}</dd></div><div><dt className="text-zinc-500">Estado</dt><dd className="mt-1 font-medium text-zinc-950">{statusLabels[detail.status]}</dd></div></dl><div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium text-zinc-950">Comprobante respaldatorio</p><p className="mt-1 text-sm text-zinc-500">{detail.receiptUrl ? 'El ticket adjunto está disponible para consulta.' : 'Este gasto todavía no tiene un comprobante adjunto.'}</p></div>{detail.receiptUrl ? <a href={detail.receiptUrl} target="_blank" rel="noreferrer"><Button variant="secondary"><FileText className="mr-2 h-4 w-4" />Ver comprobante</Button></a> : null}</div></div>{detail.notes ? <div><h3 className="text-sm font-medium text-zinc-950">Notas</h3><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-zinc-600">{detail.notes}</p></div> : null}</div> : null}
    </Modal>
