...
**No implementado / fuera de alcance de esta iteración** (documentado, no oculto): Turnos y Avisos en navegación móvil (código preservado para una negociación futura), notificaciones push reales (el token se persiste pero nada lo envía todavía), selector nativo de fecha/hora en "solicitar corrección" (campos de texto guiados), listener de conectividad en segundo plano (se chequea en los puntos de interacción), build EAS/credenciales de tienda, validación de entradas QR (deliberadamente fuera — módulo y permiso separados de Entradas Digitales).
