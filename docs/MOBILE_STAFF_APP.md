...
> Estado: **implementado end-to-end** (backend real + app Expo real). Además del fichaje, la versión 1.1 incorpora control de acceso por QR para usuarios con el permiso explícito `tickets.validate`.
...
| Escáner (condicional) | `ScannerTab > TicketPublications/TicketScanner` | `GET /tickets/publications/check-in-options`, `POST /tickets/publications/:id/check-in`, `POST /check-in/confirm` |
...
- **Validación de entradas por QR:** disponible desde la versión 1.1 únicamente si la cuenta tiene `tickets.validate`. El QR se valida online, se muestran los datos de la entrada y el operador confirma antes de consumirla mediante la transición atómica existente. No hay check-in offline. Como respaldo se admite pegar el enlace o ingresar el código impreso.
