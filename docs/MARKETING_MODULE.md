...
|---|---|
| `marketing.models.ts` | Todos los esquemas Mongoose: `MarketingSettings`, `MarketingTemplate`, `Promotion`, `MarketingAudience`, `MarketingCampaign`, `MarketingRecipient`, `MarketingUnsubscribe`, `MarketingSendLog`, `MarketingWebhookEvent` |
| `marketing-audience.service.ts` | Motor de segmentación: construye queries de Lead/Customer a partir de filtros, deduplica, excluye emails inválidos/bajas/exclusiones manuales |
...
| `unsubscribe.routes.ts` | Flujo público de baja (`/api/public/marketing/unsubscribe/:token`) |

Frontend: `apps/web/src/app/admin/marketing/**` (Resumen, Campañas, Plantillas, Promociones, Audiencias, Historial, Configuración) + `apps/web/src/features/marketing/` (editor visual de bloques, renderer HTML, tipos) + página pública `apps/web/src/app/marketing/unsubscribe/[token]/page.tsx`.
...
`MarketingRecipient` es un documento por (campaña, contacto): guarda estado (`pending → processing → sent/failed/skipped/delivered/opened/clicked/unsubscribed`), intentos, y un `unsubscribeToken` propio (UUID aleatorio, no predecible) para el enlace de baja.
...
**Decisión de diseño** — "Autorización para comunicaciones": el brief original pedía un filtro de opt-in por Lead/Customer. El código confirma que **no existe** ningún campo de consentimiento en esos modelos (ver auditoría inicial). En vez de agregar un booleano redundante que habría que mantener sincronizado en dos lugares, la autorización se resuelve enteramente por la lista `MarketingUnsubscribe` (opt-out): cualquier email presente y activo ahí queda excluido de **toda** campaña futura, sin importar de qué Lead/Customer/lista manual provenga. Es el mismo modelo que usa cualquier plataforma de email marketing real (consentimiento implícito + baja explícita), y evita que un dato pueda quedar desactualizado en el Lead/Customer mientras la baja real ya ocurrió.
...
- Tipos de bloque: logo, título, texto, imagen, botón, separador, espaciado, columnas, bloque de promoción, redes sociales, datos de contacto, pie institucional con enlace de baja.
...
Se extendieron los permisos `campaigns.*`/`promotions.*` ya reservados en `packages/shared/src/constants/permissions.ts` (existían en el enum y en `RolePresets[MANAGER]` sin ningún backend detrás) y se agregaron `marketingTemplates.*`, `marketingAudiences.*`, `marketingSettings.*`, `marketingUnsubscribes.read`. Ver §22 del brief original y `RolePresets` para el detalle por rol — `SALON_MANAGER` puede crear/editar campañas y audiencias de su propio salón pero no enviarlas ni tocar la configuración global salvo que se le otorgue el permiso puntual vía `permissionOverrides`.

## 8. Auditoría

Cada mutación relevante llama a `writeAuditLog` (creación/edición/envío/programación/cancelación/reintento de campañas, cambios de promoción/plantilla/audiencia/configuración, exportaciones). No se audita el HTML completo, solo metadata.
