...
- **Motor genérico de recordatorios (2026-08-03)**: `apps/api/src/modules/crm/reminder-engine.ts` generaliza el lock/lease/entrega de `financial-reminders.service.ts` para dominios nuevos sobre `CalendarItem` (alertas de evento, leads, presupuestos, producción, vajilla, cierre de evento, jornada abierta, liquidaciones, pagos al cliente, reseña post-evento). Todos corren dentro del mismo `/api/internal/calendar-tick` (`calendar-tick.routes.ts` recorre un array `domainTicks`) — **no crear un cron nuevo por automatización**: sumar el dominio a ese array reusando `runGenericReminderTick`. Ver `docs/MYM_EVENTOS_PROJECT_CONTEXT.md` §14 para el detalle completo y la lista de automatizaciones activas.
- Las alertas/recordatorios cargados en la pestaña "Tareas" del detalle de un Evento (`EventTasksEditor`, `resourcePlanSnapshot.alerts`) se sincronizan automáticamente hacia `CalendarItem` (`type: 'reminder'`, `source: 'event'`, `eventId` seteado) en `apps/api/src/modules/crm/event-alert-calendar-sync.service.ts`, invocado desde `POST /events` y `PATCH /events/:id` en `events.routes.ts`. Es sincronización unidireccional evento → calendario (se re-crea/borra por `metadata.eventAlertId` en cada guardado del plan); editar el ítem directamente en `/admin/calendar` no actualiza el plan del evento. **Desde 2026-08-03 estas alertas también se entregan de verdad** (antes solo se sincronizaban sin ningún consumidor): `event-alert-reminders.service.ts` las dispara por email al usuario que las creó cuando llega `remindAt`, y `POST /events` precarga 6 alertas por defecto (`event-alert-defaults.ts`) si no se mandó un plan propio. Mantener este patrón si se agregan otros orígenes de recordatorios.
